...
M049/S01 and M049/S02 are now green: `meshc init --template todo-api --db postgres <name>` emits the migration-first shared/deployable starter from S01, while `meshc init --template todo-api --db sqlite <name>` now emits the explicit local single-node starter from S02. The SQLite path passes generator, tooling, and live runtime rails; `/health` reports local SQLite state without clustered metadata; restart persistence is proven against the retained `todo.sqlite3`; and the old clustered SQLite Todo contract now lives behind the committed M047 fixture plus retained verification bundles instead of behind the public scaffold. Public docs and Mesh skills now spell the SQLite-local vs Postgres-clustered split explicitly. The remaining M049 slices still need generated `/examples`, top-level proof-app retirement, and one assembled reset verifier.
