...
M049/S01, S02, and S03 are now green: `meshc init --template todo-api --db postgres <name>` emits the migration-first shared/deployable starter from S01, `meshc init --template todo-api --db sqlite <name>` emits the explicit local single-node starter from S02, and S03 now checks in generator-owned `/examples/todo-postgres` and `/examples/todo-sqlite` as public `meshc init` output instead of hand-maintained showcase trees. The SQLite path passes generator, tooling, and live runtime rails; `/health` reports local SQLite state without clustered metadata; restart persistence is proven against the retained `todo.sqlite3`; the old clustered SQLite Todo contract now lives behind the committed M047 fixture plus retained verification bundles instead of behind the public scaffold; and the new `/examples` surface has a repo-owned materializer/check command plus retained parity/build/test bundles under `.tmp/m049-s03/`. Public docs and Mesh skills now spell the SQLite-local vs Postgres-clustered split explicitly. The remaining M049 slices are the top-level proof-app onboarding retirement and the assembled reset verifier.
